...
public class Matrix4f {
	...
	public boolean getIsIdentity() {
		...
		m.setIdentity();
		return this.getEqual(m);
	}
	
	public boolean getEqual(Matrix4f m) {
		if(this.m00 == m.m00 && this.m01 == m.m01 && this.m02 == m.m02 && this.m03 == m.m03 && 
	 ...
	 * 绕Z轴旋转矩阵
	 */
	synchronized public final void rotZ(float angle) {
		float sinAngle, cosAngle;
	...
	/**
	 * 绕任意单位轴旋转任意角度的矩阵  --> 有问题的代码。
	 */
	synchronized public final Matrix4f glRotatef(float angle, float x, float y, float z) {
		float sinAngle, cosAngle;
	 ...
	 * 缩放矩阵
	 */
	public final Matrix4f glScalef(float x, float y, float z) {
		this.setIdentity();
		this.m00 = x;
		this.m11 = y;
		this.m22 = z;	
	 ...
	 * 0 0 0 1
	 */
	public final Matrix4f glTranslatef(float x, float y, float z) {
		this.setIdentity();
		this.m03 = x;
		this.m13 = y;
		this.m23 = z;		
	...
	//axis = 1  --> y axis
	//axis = 2  --> z axis
	public final void glRotatLkj(float angle, float d, float e, float f, int axis) { 
		float sinAngle, cosAngle;
		float a, b, c; //move to orign point
	    ...
	    this.m00 = (cosAngle + (x * x) * (1 - cosAngle));
	    this.m01 = (x * y * (1 - cosAngle) + z * sinAngle);
	    this.m02 = (x * z * (1 - cosAngle) - y * sinAngle);
	    this.m03 = a * this.m00 + b * this.m01 + c * this.m02 + d;
	    		
	    this.m10 = (x * y * (1 - cosAngle) - z * sinAngle);
	    this.m10 = (cosAngle + y * y * (1 - cosAngle));
	    this.m10 = (x * y * (1 - cosAngle) + x * sinAngle);
	    this.m13 = a * this.m10 + b * this.m11 + c * this.m12 + e;
	    
	    
	    this.m20 = (x * z * (1 - cosAngle) + y * sinAngle);
	    this.m21 = (y * z * (1 - cosAngle) - x * sinAngle);
	    this.m22 = (cosAngle + z * z * (1 - cosAngle));
	    this.m23 = a * this.m20 + b * this.m21 + c * this.m22 + f;
	 ...
	 * 经过原点，任意轴旋转
	 *  
		cosx + a^2(1-cosx), ab(1-cosx) + csinx, ac(1-cosx) -bsinx, 0 
		ab(1-cosx) - csinx, b^2 + (1-b^2)cosx, bc(1-cosx) + asinx, 0
		ac(1-cosx) + bsinx, bc(1-cosx) - asinx, cosx + (1-cosx)c^2, 0
		0, 0, 1
	 */
	public final void glRotateAxis(Vector3f axis, float angle) {
		this.setIdentity();
		axis.normalize();
	    ...
	    this.m00 = (cosAngle + (u * u) * (1 - cosAngle));
	    this.m01 = (u * v * (1 - cosAngle) + w * sinAngle);
	    this.m02 = (u * w * (1 - cosAngle) - v * sinAngle);

	    this.m10 = (u * v * (1 - cosAngle) - w * sinAngle);
	    this.m10 = (cosAngle + v * v * (1 - cosAngle));
	    this.m10 = (w * v * (1 - cosAngle) + u * sinAngle);

	    this.m20 = (u * w * (1 - cosAngle) + v * sinAngle);
	    this.m21 = (v * w * (1 - cosAngle) - u * sinAngle);
	    this.m22 = (cosAngle + w * w * (1 - cosAngle));
	}
	 ...
	 *  
	 */
	public final void RotateArbitraryLine(Vector3f v1, Vector3f v2, float theta)
	{
	    float a = v1.x;
	    float b = v1.y;
	    float c = v1.z;
	    ...
	    m00 = uu + (vv + ww) * costheta;
	    m01 = uv * (1 - costheta) + w * sintheta;
	    m02 = uw * (1 - costheta) - v * sintheta;


	    m10 = uv * (1 - costheta) - w * sintheta;
	    m11 = vv + (uu + ww) * costheta;
	    m12 = vw * (1 - costheta) + u * sintheta;
	    

	    m20 = uw * (1 - costheta) + v * sintheta;
	    m21 = vw * (1 - costheta) - u * sintheta;
	    m22 = ww + (uu + vv) * costheta;
	    
	    m30 = (a * (vv + ww) - u * (bv + cw)) * (1 - costheta) + (bw - cv) * sintheta;
	    m31  = (b * (uu + ww) - v * (au + cw)) * (1 - costheta) + (cu - aw) * sintheta;
	    m32  = (c * (uu + vv) - w * (au + bv)) * (1 - costheta) + (av - bu) * sintheta;
...
*/
	...
	final void invertGeneral(Matrix4f m1) {
		...
		if (!luDecomposition(temp, row_perm)) {
			...
			//lkj throw new RuntimeException("Matrix4f12");
		}
	 ...
	 * @param buffer
	 */
	 synchronized public final void fillFloatBuffer(FloatBuffer buffer) {
		buffer.position(0);
		buffer.put(m00);
		buffer.put(m10);
		buffer.put(m20);
	...
	public String toString() {
		...
		*/
		builder.append("\n  ");
		builder.append(m00);builder.append(",");builder.append(m01);builder.append(",");
		builder.append(m02);builder.append(",");builder.append(m03);builder.append(",");
		builder.append("\n  ");
		
		builder.append(m10);builder.append(",");builder.append(m11);builder.append(",");
		builder.append(m12);builder.append(",");builder.append(m13);builder.append(",");
		builder.append("\n  ");
		
		builder.append(m20);builder.append(",");builder.append(m21);builder.append(",");
		builder.append(m22);builder.append(",");builder.append(m23);builder.append(",");
		builder.append("\n  ");
		
		builder.append(m30);builder.append(",");builder.append(m31);builder.append(",");
		builder.append(m32);builder.append(",");builder.append(m33);builder.append(",");
		builder.append("\n  ");
		builder.append(" ]");
